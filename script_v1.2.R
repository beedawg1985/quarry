...
getLidar2 <- function(bufferedPoly,overwrite=T) {
  ...
  names(st) <- paste0('req',toVrt$req_id,'_',
                      toVrt$year)
  # subtract earliest survey from latest survey
  ...
  a <- st$req1_2018 - st$req1_2007
  writeRaster(st$req1_2018,'test2018.tif',options=co, overwrite=T)
  writeRaster(st$req1_2007,'test2007.tif',options=co, overwrite=T)
  writeRaster(a,'test.tif',options=co, overwrite=T)
  # split by year
  yearVrts <- split.data.frame(tIndex,tIndex$year)
  
  # build VRTs
  lapply(names(yearVrts), function(x) {
    vrtOut <- paste0(folderPath,'/',
                     basename(folderPath),
                     '_',x,'.vrt')
    gdalbuildvrt(yearVrts[[x]]$location, 
                 output.vrt=vrtOut)
    r <- raster(vrtOut)
    tifOut <- paste0(folderPath,'/',
                     basename(folderPath),
                     '_',x,'.tif')
    writeRaster(r,tifOut,options=c('COMPRESS=DEFLATE',
                                   'TILED=YES'))
    gdaladdo(tifOut, levels=c('8','16','32','64','128'))
  })
  
  # search for changes in elevation over mutually overlapping maps
  # which request polygons intersect with which tiles
  bufferedTiles <- st_join(bufferedPoly, tIndex,
                           left=F)

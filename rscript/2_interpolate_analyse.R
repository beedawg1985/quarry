...
# x <- 1
rasData <- lapply(1:nrow(int), function(x) {
  pol <- int[x,]
  ...
  intA <- interpolateRas(foldA$train,foldA$all, maskPoly = pol, cv=T)
  ...
  save(dat, file=paste0('/media/mal/working_files/quarry/rasdata_10feb_introw_',x,'.RData') )
})
...
f <- list.files('/media/mal/working_files/quarry',full.names = T,
                pattern='rasdata_10feb_introw*')[1:5]
rasData <- lapply(f, function(x) { rmses <- loadRData(x) })

# helper function extracts relevant data from compareInt() output
# x <- names(rasData[[1]]$orig.maps)[1]
paramsCv <- lapply(names(rasData[[1]]$orig.maps), function(x) {
  intParam <- rasData %>% 
    map_df(~.x$orig.maps[[x]]$param, .default=NA)
  ...
  if (nrow(intParam) > 0) {
    ...
    paramsCv <- intParam %>% 
      left_join(allRmse,on=c('run_no','intpol_fid','int_method'))
  } else return(NULL)
})
names(paramsCv) <- names(rasData[[1]]$orig.maps)
paramsCv <- paramsCv %>% discard(is.null)
